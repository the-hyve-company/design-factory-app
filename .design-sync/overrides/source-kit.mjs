...
// forked from design-sync lib/source-kit.mjs — only change vs upstream: 'dfds'
// added to GENERIC_DIR so the src/components/dfds primitives recategorize via
// cfg.docsMap (Controls/Buttons) instead of all landing in a "dfds" group.
// Relative imports repointed to the staged lib; ts-morph to the staged dist.
...
const GENERIC_DIR = new Set(['components', 'component', 'src', 'lib', 'ui', 'packages', 'react', 'dfds']);
